            ...
            >
                ...
                >
                    <div className="text-center">
                        <p className="text-[8px] font-bold text-[#780606] uppercase tracking-widest mb-1">View</p>
...
const MAX_SCROLL = 800; // Virtual scroll range (reduced for faster animation)
...
export default function IntroAnimation() {
    ...
    useEffect(() => {
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            // Multiply deltaY by 3 for faster touch scrolling
            const newScroll = Math.min(Math.max(scrollRef.current + deltaY * 3, 0), MAX_SCROLL);
            scrollRef.current = newScroll;
            virtualScroll.set(newScroll);
        };
    ...
    // Happens between scroll 0 and 200 (faster)
    const morphProgress = useTransform(virtualScroll, [0, 200], [0, 1]);
    const smoothMorph = useSpring(morphProgress, { stiffness: 80, damping: 15 });

    // 2. Scroll Rotation (Shuffling): Starts after morph (e.g., > 200)
    // Rotates the bottom arc as user continues scrolling
    const scrollRotate = useTransform(virtualScroll, [200, 800], [0, 360]);
    const smoothScrollRotate = useSpring(scrollRotate, { stiffness: 80, damping: 15 });
    ...
    // --- Intro Sequence (only starts when in view) ---
    useEffect(() => {
        if (!isInView) return;
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full bg-white dark:bg-black overflow-hidden">
            {/* Container */}
            <div className="flex h-full w-full flex-col items-center justify-center perspective-1000">
                ...
                <div className="absolute z-0 flex flex-col items-center justify-center text-center pointer-events-none top-1/2 -translate-y-1/2">
                    <motion.h1
                        ...
                        className="text-2xl font-medium tracking-tight text-gray-800 dark:text-white md:text-4xl"
                    >
                    ...
                    <motion.p
                        ...
                        className="mt-4 text-xs font-bold tracking-[0.2em] text-gray-500 dark:text-gray-400"
                    >
                        SCROLL TO EXPLORE
                    </motion.p>
                </div>
